...
// check MUMmer
system('which nucmer promer delta-filter show-coords &> /dev/null &', $retval);
if($retval != 0) pErr('MUMmer\'s programs are unreachable in your $PATH');
 
$totalTimeStart = getMicrotime();
$opt = getopt("t:r:o:h", array('nuc', 'pro', 'dotplot', 'time', 'coords:', 'c1:', 'c2:', 'noFna', 'debug', 'getWeight', 'unmap'));
...
$getWeight = isset($opt['getWeight']) ? 1 : 0;
$genUnmapped = isset($opt['unmap']) ? 1 : 0;
$noFna = isset($opt['noFna']) ? 1 : 0;
if($genUnmapped == 1) $noFna = 0;
...
//	$mumOptSuf = $mum.".maxM";
	...
	$filter = "$refCurrentPath.$mumOptSuf.filter";
...
foreach($arr as $k => $line){
	...
	// get weight: number of exact matches of LEN 1
	$weight += $element[6];
...
$getWeightTime = getMicrotime();
if($getWeight == 1){
	// to get number of exact matches of LEN 1
	$indel = 0;
	$mismatch = 0;
	$filter_arr = file($filter, FILE_IGNORE_NEW_LINES);
	if($filter_arr[1] == 'PROMER') $weight /= 3;
	foreach($filter_arr as $k => $line){
		if($line[0] != '-' && !is_numeric($line[0])) continue;
		$var = preg_split("/ +/", $line);
		if(isset($var[4])) $mismatch += $var[4]; // $var[4]: number of errors
		if($line[0] == '-') $indel++;
	}
	$weight -= $mismatch;
	$weight += $indel;
	
	file_put_contents("$outPath/$mum.coords.weight", $weight);
}
$getWeightTime = round(getMicrotime() - $getWeightTime, 3);
...
if($target != ''){
	...
	if($noFna == 0){
		...
		//	genFnaFile($ref, $outputFile_ref);
	}

	if($genUnmapped == 1){
		genUnmappedFile($target, $tar_originMarkers, $scaffoldNum[1], $outputFile_target, '');
		//	genUnmappedFile($ref, $ref_originMarkers, $scaffoldNum[0], $outputFile_ref, "ref.");
		//	genUnmappedFile_joinAll($target, $tar_originMarkers, $scaffoldNum[1], $outputFile_target, "");
	}
...
if($getRunTime == 1){
	...
	if($noFna == 0)	$runtimeContext .= "Generate the fasta file: $genSeqTime seconds\n";
	if($getWeight == 1)	$runtimeContext .= "Get weight: $getWeightTime seconds\n";
